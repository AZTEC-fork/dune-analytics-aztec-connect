...
CREATE
OR REPLACE FUNCTION dune_user_generated.f_bytea_to_bit(IN i_bytea BYTEA) RETURNS BIT VARYING AS $ BODY $ DECLARE w_bit BIT VARYING := b '';

BEGIN w_bit := ('x' || ltrim(i_bytea :: text, '\x')) :: bit varying;
...
CREATE
OR REPLACE FUNCTION dune_user_generated.f_bit_to_bytea(IN i_bit BIT VARYING) RETURNS bytea AS $ BODY $ DECLARE w_panel_data_len INTEGER;
...
IF length(i_bit) % 8 != 0 THEN RAISE 'Can not convert to bytea. The passed argument is % bits',
length(i_bit);

END IF;

FOR i IN 0..w_panel_data_len - 1 LOOP w_str_bit := w_str_bit || lpad(
  to_hex(
    substring(
      i_bit
      from
...
CREATE
OR REPLACE FUNCTION dune_user_generated.f_bytea_rshift(IN i_bytea BYTEA, IN i_num INTEGER) RETURNS BYTEA AS $ BODY $ DECLARE w_bit BIT VARYING := b '';

w_bytea BYTEA := null :: BYTEA;

BEGIN w_bit := dune_user_generated.f_bytea_to_bit(i_bytea);
...
create
or replace function dune_user_generated.fn_process_aztec_block(data bytea) returns dune_user_generated.aztec_v2_proof_data_struct as $ $ declare proofData dune_user_generated.aztec_v2_proof_data_struct;
...
begin rollupSize = bytea2numeric(substring(data, 61, 4), false);

for i in 0..NUM_BRIDGE_CALLS_PER_BLOCK - 1 loop bridgeId = substring(data, startIndex + 1, 32);
...
if (get_bit(bitConfig, 0) = 1) then secondInputInUse = true;

else secondInputInUse = false;

end if;

if (get_bit(bitConfig, 1) = 1) then secondOutputInUse = true;

else secondOutputInUse = false;
...
select
  --   addressId: numeric
  bytea2numeric(
    substring(bridgeId, length(bridgeId) - 3, 4),
    false
  ) :: bigint & ((1 :: bigint < < 32) - 1),
  --   inputAssetIdA: numeric
  bytea2numeric(
    substring(inputAssetIdA, length(inputAssetIdA) - 7, 8),
    false
  ) :: bigint & (((1 :: bigint < < 30) - 1)),
  --   outputAssetIdA: numeric
  bytea2numeric(
    substring(outputAssetIdA, length(outputAssetIdA) - 7, 8),
    false
  ) :: bigint & (((1 :: bigint < < 30) - 1)),
  --   inputAssetIdB: numeric,
  bytea2numeric(
    substring(inputAssetIdB, length(inputAssetIdB) - 7, 8),
    false
  ) :: bigint & (((1 :: bigint < < 30) - 1)),
  --   outputAssetIdB: numeric,
  bytea2numeric(
    substring(outputAssetIdB, length(outputAssetIdB) - 7, 8),
    false
  ) :: bigint & (((1 :: bigint < < 30) - 1)),
  --   auxData: numeric
  bytea2numeric(
    substring(auxData, length(auxData) - 7, 8),
    false
  ) :: bigint & (((1 :: bigint < < 62) - 1)),
  --  secondInputInUse: boolean
  secondInputInUse,
  --  secondOutputInUse: boolean
  secondOutputInUse into bridge;
...
for i in 0..NUMBER_OF_ASSETS - 1 loop assetIds = array_append(
  assetIds,
...
for i in 0..NUMBER_OF_ASSETS - 1 loop totalTxFees = array_append(
  totalTxFees,
  bytea2numeric(substring(data, startIndex + 1, 32), false)
);
...
for i in 0..NUM_BRIDGE_CALLS_PER_BLOCK - 1 loop defiInteractionNotes = array_append(
...
innerProofDataLength = bytea2numeric(
  substring(data, innerProofStartIndex + 1, 4),
  false
);

innerProofStartIndex = innerProofStartIndex + 4;

while innerProofDataLength > 0 loop innerProofByteData = substring(data, innerProofStartIndex, length(data));

proofId = bytea2numeric(
  substring(data, innerProofStartIndex + 1, 1),
  false
);
...
case
  assetId
  when 0 then asset = 'Ethereum';

when 1 then asset = 'DAI';

when 2 then asset = 'wstETH';
...
if proofId = 1 then proofType = 'DEPOSIT';

else proofType = 'WITHDRAW';
...
4,
5,
6 then if proofId = 3 then proofType = 'SEND';

elsif assetId = 4 then proofType = 'ACCOUNT';

elsif assetId = 5 then proofType = 'DEFI_DEPOSIT';

else proofType = 'DEFI_CLAIM';
...
select
  ...
  -- numRollupTxs
  bytea2numeric(
    substring(data, startIndex + 32 + 32 + 1, 32),
    false
  ),
  -- innerProofs
  innerProofs into proofData;
